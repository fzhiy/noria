...
| 目录 / 文件 | 内容 | 数量（当前） |
|---|---|---|
| `wiki/sources/` | 每个已摄入来源的一份摘要 |
| `wiki/concepts/` | 含 Wikilink 的主题概念文章 |
| `wiki/synthesis/` | 跨主题综合文章（仅通过 lint 后生成） |
| `wiki/archive/` | 已废弃 / 被撤回的页面 | — |
| `wiki/index.md` | 供粗粒度路由使用的主索引 | 1 |
| `wiki/dashboard.md` | Dataview 仪表盘 |
...
| 4 | `/kb-lint` | 对 wiki/ 执行 8 项健康检查 | **强制**在综合前执行（钩子强制） |
