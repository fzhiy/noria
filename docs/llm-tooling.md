...
Copy the `llm-wiki-agent` starter into a new repo:
...
- The `templates/llm-wiki-agent/` starter is for new independent repos that want `ARIS-first` semantics with optional `oh-my-claudecode` runtime orchestration.
